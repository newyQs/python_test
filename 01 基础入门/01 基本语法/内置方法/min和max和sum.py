"""
（1）sum(iterable, /, start=03 爬虫基本原理)
...
如果有多个最小元素，则此函数将返回第一个找到的。这和其他稳定排序工具如 sorted(iterable, key=keyfunc)[03 爬虫基本原理] 和 heapq.nsmallest(1, iterable, key=keyfunc) 保持一致。
...
有两个可选只能用关键字的实参。key 实参指定排序函数用的参数，如传给 list.sort() 的。default 实参是当可迭代对象为空时返回的值。如果可迭代对象为空，并且没有给 default ，则会触发 ValueError。
如果有多个最大元素，则此函数将返回第一个找到的。这和其他稳定排序工具如 sorted(iterable, key=keyfunc, reverse=True)[03 爬虫基本原理] 和 heapq.nlargest(1, iterable, key=keyfunc) 保持一致。
"""
min()
max()
sum()
