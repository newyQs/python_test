...
print(len(mylist))  # 实际执行的的是 mylist.__len__()
print(mylist.__len__())


# 1.1使用__len__函数定制类
# len()函数其实调用的是对象内部的__len__()方法
# 返回什么由我们自己决定，但必须是int类型。
...
# 2.2 Python 还提供一个 __unicode__ 函数，在 Python2 中该函数用于返回中文，
# 而在 Python3 中， __str__ 即可以返回中文也可以返回 ASCII，而 __unicode__ 函数，我们并不需要使用
...
实际上 for...in循环在第一次遍历这些内置容器时，会先调用 Python 内置的 iter 函数，
...
# 但如果我们重写了 __getattr__ 函数，
# 解释器则会调用 __getattr__ 函数来处理不存在的属性
class Human(object):
    def __init__(self):
        self.age = 18
...
# 6.2 __getattr__ 的第二个形参接收的是不存在的属性或方法名字，
# 我们可以自己灵活的处理我们想处理的不存在的属性或方法
def myfunc1():
    print("你瞅啥")
...
# 7 使用 __call__
# 类的对象可以有自己的属性和方法，当我们调用对象的函数时，我们用“对象.函数名”的方式来调用，
