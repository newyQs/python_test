...
# 1.3给类绑定呢？
# 要注意 __slots__ 无法限制类的属性，定义类或动态绑定类的属性都是有效的
Human.age = 22
print(ruhua.age)
...
2.给类定义__slots__之后，在类的内部或外部对实例对象绑定任何其他属性都不起作用；
3.__slots__只对实例对象的属性有所限制，而不限制类的属性；
4.__slots__只对当前定义的类有效，如果想要继承的子类有效，则需要在子类中也定义__slots__，这样子类中
 __slots__限制在子类+父类的__slots__总和；
5.如果子类中定义了__slots__，而父类没有定义__slots__，则该__slots__对子类和父类都不起作用；

子类定义，父类未定义：子类和父类都不起作用
子类未定义，父类定义：子类不起作用
子类和父类都定义：子类限制在子类+父类定义之和
'''
