...
'''
1.具有__iter__()方法的对象称为可迭代对象，该方法可获取其迭代器对象；
2.具有__iter__()和__next__()方法的对象称为迭代器对象，该方法能够自动返回下一个结果，当达到序列结果，引发Stopiteration异常
3.
...
iter( ) 函数与 __iter__() 方法联系非常紧密，iter()是直接调用该对象的 __iter__() 方法，并将其返回结果作为自己的返回值，next( ) 函数则是调用该对象的 __next__() 方法获取当前元素
'''
